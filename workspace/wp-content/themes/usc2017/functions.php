...
function testtheme_scripts() {
	wp_enqueue_style( 'testtheme-purecss', get_template_directory_uri() . '/css/pure-min.css' );
	wp_enqueue_style( 'testtheme-gridsresponsive', get_template_directory_uri() . '/css/grids-responsive-min.css' );
 ...
 * Custom post types.
 */
require get_template_directory() . '/inc/cpt-model.php';

require get_template_directory() . '/inc/cpt-cars.php';

require get_template_directory() . '/inc/cpt-pod.php';

require get_template_directory() . '/inc/cpt-memberships.php';

require get_template_directory() . '/inc/cpt-steps.php';

/**
 * Custom taxonomies.
 */
require get_template_directory() . '/inc/tax-fleet.php';

/* Used to delete taxonomy lists. Ran because economic car search filter was not working i.e. not showing any cars */
/* function sjc_delete_terms() {
     if ( is_admin() ) {
          $terms = get_terms( 'fleet', array( 'fields' => 'ids', 'hide_empty' => false ) );
          foreach ( $terms as $value ) {
               wp_delete_term( $value, 'fleet' );
          }
     }
}
add_action( 'init', 'sjc_delete_terms' ); */

/* jQuery integration. Overrides WP's and replaces with custom version. Also adds custom scripts. */
function shapeSpace_include_custom_jquery() {
	...
	wp_register_script('usc', get_template_directory_uri() . '/js/usc.js', array( 'jquery'));
	wp_enqueue_script('usc');

}
add_action('init', 'shapeSpace_include_custom_jquery');

/* Allows steps to be reordered in back-end for display in front-end */
function posts_order_wpse_91866() 
{
    add_post_type_support( 'step', 'page-attributes' );
}
